    ...
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4">
        {fileCategories.map((category, index) => (
          <div key={index} className={`flex justify-between border-b border-stroke px-7.5 py-7 ${index % 2 === 1 ? "xl:border-b-0" : ""} ${index < fileCategories.length - 2 ? "border-r dark:border-strokedark" : ""}`}>
            <div className="flex items-center gap-5.5">
              <div className={`flex h-15 w-14.5 items-center justify-center rounded-lg ${category.bg}`}>
                {category.icon}
              </div>
              <div>
                <p className={`text-lg font-medium`} style={{ color: category.color }}>{category.title}</p>
                <span className="font-medium">{category.count}</span>
              </div>
            </div>
            <div>
              <p className="mt-1.5 font-medium text-black dark:text-white">{category.size}</p>
            </div>
          </div>
        ))}
      </div>
